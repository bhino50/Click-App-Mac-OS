...
# MARKETING_VERSION is the single artifact-version source of truth. Refuse to
# package if the project configurations disagree rather than naming a DMG with
# a version that differs from the app bundle.
VERSION_VALUES="$(
  /usr/bin/awk -F ' = ' '
...
ENTITLEMENTS="$ROOT_DIR/Click/Click.entitlements"
...
if [[ -n "${DEVELOPER_ID:-}" && -n "${NOTARY_PROFILE:-}" ]]; then
  ...
  APP_ARCHIVE_PATH="$DIST_DIR/$APP_NAME-$VERSION-macOS14-notarized.zip"
...
manifest["downloadURL"] = (
    "https://github.com/bhino50/Click-App-Mac-OS/releases/download/"
    f"v{version}/Click-{version}.dmg"
)
manifest.setdefault("notes", f"Click {version}.")
