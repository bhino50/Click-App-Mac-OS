...
/// First-run onboarding. Walks the user through granting Accessibility access
...
struct PermissionsView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            Text("To play sounds as you type, Click needs **Accessibility** access. Keystrokes are never stored or transmitted.")
                .font(.body)

            stepRow(
                index: 1,
                title: "Open System Settings",
                detail: "We'll deep-link you to Privacy & Security → Accessibility.")
            stepRow(
                index: 2,
                title: "Enable Click",
            ...
            HStack {
                Button {
                    // Register Click in the TCC list (the only way to add it),
                    // then deep-link straight to the Accessibility pane.
                    // The native "wants accessibility access" dialog appears
                    // briefly — but it's user-initiated, so it doesn't feel
                    // like a surprise stacked on top of this window.
